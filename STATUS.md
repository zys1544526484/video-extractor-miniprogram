...
- M1 P3 大源文件公开复验修正（2026-09-09）：用户在 `64a9b1b9bf1f1a1acb3964f2e482f636b429090c` 上完成新的权威 Headed 验收。目标作品 `7678969660380843304`、canonical 页面、唯一可见 blob 主播放器和 4 个 origin-only `video/mp4` 响应均成立；`candidate_source=main_player_mse`、MSE 采样 4 次且 1 次匹配，形成唯一视频缓冲、唯一绑定组和唯一等价组，证明主播放器媒体归属已经成功。随后 `SafeHttpClient.probe_media` 错用最终微信成品 `180MiB` 上限，在无 Cookie Range 读取前返回 `MEDIA_TOO_LARGE`，因此该样本仍未完成 1024-byte 公开复验，不能计为抖音 PASS。`6e42163dfd77ef9a2b50de0a309ab9929fe86a7f` 将独立 session smoke 的探测上限改为既有 `MAX_SOURCE_VIDEO_BYTES`（默认 `2GiB`）；最终成品上限仍为 `180MiB`，超限源后续必须由媒体管道压缩，源本身超过 `2GiB` 仍拒绝。Worker 失败诊断现在将后置探测/Range 阶段准确记为 `media_verify`。本地全量验证为后端 `239 passed`（2 warnings）、Ruff/compileall、Alembic 空库升级/head、后端 production 配置、Node `49 passed`、小程序常规/production 各 `80 files checked` 与 `git diff --check` 全部 PASS；[GitHub Actions run 34314446345](https://github.com/zys1544526484/video-extractor-miniprogram/actions/runs/34314446345) 已成功（含 Docker build 与 Caddy validate）。修复后的真实 1024-byte 读取仍为 `NOT VERIFIED`。
