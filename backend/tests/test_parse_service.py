...
class LargeSourceHttp(FastPathHttp):
    def __init__(self, payload: bytes) -> None:
        super().__init__(size=len(payload))
    ...
    async def open_stream(self, url, *, headers=None, range_header=None):
        self.stream_calls += 1
        self.ranges.append(range_header)
        return OpenedChunkStream(ChunkResponse([self.payload], status_code=200))
...
@pytest.mark.asyncio
async def test_parse_materializes_source_above_final_limit_for_bounded_processing(
    tmp_path: Path,
) -> None:
    settings = Settings(
        app_env="test",
        temp_dir=tmp_path / "media",
        max_video_bytes=8,
        max_source_video_bytes=32,
        min_free_disk_bytes=0,
    )
    settings.temp_dir.mkdir(parents=True)
    database = Database(f"sqlite:///{tmp_path / 'large-source.db'}")
    database.create_schema()
    with database.session_factory() as session:
        session.add(User(openid="large-source-user"))
        session.commit()
    http = LargeSourceHttp(b"source-larger-than-final")
    service = ParseService(
        settings,
        http,
        FastPathRegistry(),
