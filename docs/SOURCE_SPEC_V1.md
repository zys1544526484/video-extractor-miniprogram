...
一个微信小程序，用户粘贴公开视频分享链接后，在使用权限有效时自动解析公开可访问的媒体资源，并允许用户预览和保存自己有权保存的视频；未解锁用户完整观看一次激励视频广告后获得 24 小时使用权限。
...
→ 检查权限
→ 未解锁
→ 弹窗说明“观看一次广告，解锁 24 小时”
→ 用户确认
→ 激励视频广告
→ 完整观看
→ 服务端记录 `unlock_until = server_now + 24h`
→ 自动继续原请求
→ 解析中
→ 结果页
→ 预览
...
E2E-01 首次用户：
登录 → 粘贴 → 开始 → 广告 → 完整观看 → 解锁 → 解析 → 结果 → 下载 → 保存。
